...
class Countries extends Component {
  ...
  getVisitCountry = details => {
    const isVisited = !details.isVisited
    this.setState(prevState => ({
      countriesList: prevState.countriesList.map(eachCountry => {
        if (eachCountry.id === details.id) {
          return {
            ...eachCountry,
            isVisited,
          }
    ...
    this.setState(prevState => ({
      visitedCountriesList: prevState.countriesList.filter(
        eachCountry => eachCountry.isVisited === true,
      ),
    }))
  }

  removeVisitedCountry = details => {
    const isVisited = !details.isVisited
    this.setState(prevState => ({
      countriesList: prevState.countriesList.map(eachCountry => {
        if (eachCountry.id === details.id) {
    ...
    this.setState(prevState => ({
      visitedCountriesList: prevState.visitedCountriesList.filter(
        eachCountry => eachCountry.id !== details.id,
      ),
    }))
  }
  ...
  render() {
    ...
    return (
      <CountryContainer>
        <CountriesTitle>Countries</CountriesTitle>
        <CountriesListContainer>
          {countriesList.map(eachItem => (
            <CountryItem
              countryDetails={eachItem}
              key={eachItem.id}
              getVisitCountry={this.getVisitCountry}
            />
          ))}
        </CountriesListContainer>
        <CountriesTitle>Visited Countries</CountriesTitle>
